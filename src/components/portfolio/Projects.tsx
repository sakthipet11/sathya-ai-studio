...
function ArchitectureViz({ slug }: { slug: string }) {
  ...
  return (
    <div className="py-1" aria-label={`Architecture flow: ${flow.join(", then ")}`}>
      <div className="mb-3 text-[10px] font-medium uppercase tracking-[0.16em] text-primary/80">
        Delivery flow
      </div>
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 sm:items-center">
        {flow.map((label, index) => (
          <div key={label} className="relative">
            <div className="flex min-h-14 items-center justify-center rounded-xl border border-primary/25 bg-primary/[0.06] px-3 text-center text-xs font-medium text-foreground/90">
              {label}
            </div>
            {index < flow.length - 1 && (
              <ArrowRight
                aria-hidden
                className="absolute -right-3 top-1/2 z-10 hidden size-4 -translate-y-1/2 text-primary sm:block"
              />
            )}
          </div>
        ))}
...
function ProjectCard({ p, featured }: { p: (typeof PROJECTS)[number]; featured?: boolean }) {
  return (
    <article
      className={cn(
        "group relative overflow-hidden rounded-3xl glass p-6 transition-[transform,box-shadow] duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-primary/10 md:p-7 h-full",
    ...
    >
      ...
      <div className="inline-flex items-center gap-1.5 rounded-full glass px-2 py-0.5 text-[10px] uppercase tracking-widest text-primary">
        <Sparkles className="size-2.5" /> {p.category} initiative
      </div>
      <h3 className="mt-3 font-display text-xl font-semibold md:text-2xl">{p.name}</h3>
      <p className="mt-1 text-sm text-muted-foreground">{p.tagline}</p>
      {featured && (
        <div className="mt-5 rounded-2xl border border-white/10 bg-black/20 px-4 py-3 md:px-5">
          <ArchitectureViz slug={p.slug} />
        </div>
      )}
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        <div>
          <div className="text-[10px] uppercase tracking-widest text-muted-foreground">Context</div>
    ...
    <section id="projects" className="relative py-16 md:py-20 scroll-mt-24">
      <div className="mx-auto max-w-6xl px-4">
        <ScrollReveal>
          <SectionHeader
            eyebrow="Projects"
            title="Selected enterprise initiatives."
            description="A concise view of verified initiatives. Client-specific implementation details remain confidential."
          />
        </ScrollReveal>
